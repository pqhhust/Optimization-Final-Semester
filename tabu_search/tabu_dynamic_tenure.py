...
class TabuSolver:
    def __init__(self, N, M, K, a, b, c, d, e, f, s_matrix, g_matrix, t_list):
        ...
        # Cấu hình Tabu
        self.max_iter = 1000
        # Thay vì cố định self.tabu_tenure = 15, ta dùng dải động:
        self.min_tabu_tenure = 20   # Khi đang tìm thấy lời giải tốt (Khai thác)
        self.max_tabu_tenure = 80   # Khi bị bí (Khám phá xa hơn)
        self.tabu_tenure = self.min_tabu_tenure # Khởi tạo bằng min
        
        self.stagnation = 0         # Đếm số vòng lặp không tìm thấy kỷ lục mới
        self.tabu_list = {}
    ...
    def solve(self):
        ...
        for iteration in range(self.max_iter):
            ...
            if best_move:
                ...
                if improved:
                    # Nếu tìm thấy kỷ lục mới -> Đang đi đúng hướng -> Tập trung khai thác
                    self.stagnation = 0
                    self.tabu_tenure = self.min_tabu_tenure 
                else:
                    # Nếu không cải thiện -> Tăng sự trì trệ
                    self.stagnation += 1
                    
                    # Cứ mỗi vòng không tiến bộ, tăng Tabu Tenure lên 2 đơn vị
                    # Nhưng không được vượt quá max_tabu_tenure
                    # Việc này ép thuật toán cấm nhiều hơn, buộc phải đi xa hơn
                    if self.tabu_tenure < self.max_tabu_tenure:
                        self.tabu_tenure += 2
