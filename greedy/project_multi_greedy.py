...
def solve():
    ...
    def run_one_pass():
        ...
        council_indices = list(range(K)) # 0..K-1

        # --- GIAI ĐOẠN 1: ĐIỀN ĐẾN MỨC TỐI THIỂU (Min a, Min c) ---
        # Mục đích: Đảm bảo không hội đồng nào bị thiếu người
        for k_idx in council_indices:
            curr_council = councils[k_idx]
            
            # 1.1 Chọn SEED (ĐÂY LÀ CHỖ DUY NHẤT CÓ RANDOM)
        ...
        # --- GIAI ĐOẠN 2: ĐIỀN ĐẾN MỨC TỐI ĐA (Max b, Max d) ---
        ...
        # --- GIAI ĐOẠN 3: VÉT CẠN (POST-PROCESSING) ---
        # Cố gắng nhét nốt những người còn sót vào bất kỳ đâu hợp lệ
        # Ưu tiên nơi có gain cao nhất (Best Fit)
    ...
    # ---------------------------------------------------------
    # 4. HÀM TÍNH ĐIỂM (CHỈ DÙNG ĐỂ SO SÁNH CUỐI CÙNG)
    # ---------------------------------------------------------
